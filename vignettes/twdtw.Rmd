...
```{r twdtw-example, echo = FALSE, eval = TRUE, fig.path='figure/', fig.width=page_width, fig.align='center', fig.cap='Matches of the temporal pattens $\\mathcal{Y}$ to the long-term time series $\\mathcal{Y}$.'}
aux = alig2
...
```{r twdtw-workflow, echo = FALSE, eval = TRUE, fig.path='figure/', fig.width=page_width, fig.height=page_width/12, fig.align='center', fig.cap='Workflow to find the alignments between two time series using TWDTW.'}
library(png)
library(grid)
...
```{r time-weight-functions, echo = FALSE, eval = TRUE, fig.path='figure/', fig.width=page_width, fig.align='center', fig.cap='Illustration of liner time-weight (dashed line) and logistic time-weight (solid line). The linear function implies high cost for small time differences, while the logistic function is more flexible for small time differences.'}
...
```{r local-cost-matrix, echo = FALSE, eval = TRUE, fig.path='figure/', fig.width=page_width, fig.align='center', fig.cap='Illustration of DTW local cost matrix (top) and Time-Weighted DTW local cost matrix (bottom). The dark areas in the matrices indicate the low-cost candidate paths.'}
...
```{r cost-matrix, echo = FALSE, eval = TRUE, fig.path='figure/', fig.width=page_width, fig.align='center', fig.cap='Illustration of the DTW accumulated cost matrix (top) and the Time-Weighted DTW accumulated cost matrix (bottom). The dark color "valleys" indicate the low-cost paths, *i.e* the candidates to match the temporal pattern to the long-term time series.'}
...
```{r minimum-path-twdtw, include = FALSE, echo = FALSE, eval = TRUE, fig.path='figure/', fig.width=page_width, fig.align='center', fig.cap='Example of paths in the the accumulated cost matrix $\\mathbf{D}$ after the reverse algorithm. The red lines are the low cost paths, the white lines indicate the start and end of the subintervals of $\\mathcal{X}$, $a$ and $b$ are the $jth$ indices of start and end of the alignments, respectively.'}
n = 1:nrow(getAlignments(alig2))
df.path = do.call("rbind", lapply(n, function(i){
...
df.text$value = df.text$value + c(rep(-2, length(n)), rep(3, length(n)))
df.text$index1 = df.text$index1 + c(rep(-max(df.path$index1)+2, length(n)), rep(-.4, length(n)))
...
```{r minimum-paths-dtw-twdtw, echo = FALSE, eval = TRUE, fig.path='figure/', fig.width=page_width, fig.align='center', fig.cap='Example of paths in the the accumulated cost matrix $\\mathbf{D}$ after the reverse algorithm. The red lines are the low cost paths, the white lines indicate the start and end of the subintervals of $\\mathcal{X}$, $a$ and $b$ are the $jth$ indices of start and end of the alignments, respectively.'}
